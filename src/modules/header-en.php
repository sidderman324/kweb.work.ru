<header class="page-header">
	<div class="container page-header__inner">

		<a href="/en/" class="logo">
			<img src="/img/kweb_logo_black.png" alt="" class="logo__img">   
		</a>

		<nav class="main-menu">
			<ul class="main-menu__list">
				<li class="main-menu__item <?php echo $service; ?>"><a class="main-menu__link main-menu__link--hidden">Service</a>
					<ul class="submenu">
						<li class="submenu__item"><a href="/en/ico_service/" class="submenu__link">ICO</a></li>
						<li class="submenu__item"><a href="/en/web_service/" class="submenu__link">WEB studio</a></li>
					</ul>
				</li>
				<li class="main-menu__item <?php echo $dev; ?>"><a class="main-menu__link main-menu__link--hidden">Development</a>
					<ul class="submenu">
						<li class="submenu__item"><a href="/en/dev/websitedev.php" class="submenu__link">Website development</a></li>
						<li class="submenu__item"><a href="/en/dev/seo.php" class="submenu__link">SEO</a></li>
						<li class="submenu__item"><a href="/en/dev/smm.php" class="submenu__link">SMM</a></li>
						<li class="submenu__item"><a href="/en/dev/marketing.php" class="submenu__link">Web marketing</a></li>						
					</ul>
				</li>							
				<li class="main-menu__item <?php echo $portfolio; ?>"><a href="/en/portfolio/" class="main-menu__link">Portfolio</a>
				</li>
				<li class="main-menu__item <?php echo $team; ?>"><a href="/en/team/" class="main-menu__link">Team</a></li>
				<li class="main-menu__item <?php echo $contacts; ?>"><a href="/en/contacts/" class="main-menu__link">Contacts</a></li>
			</ul>
		</nav>

		<a href="tel:+78002500025" class="page-header__phone">8 900 253 87 87</a>

		<a href="/" class="page-header__lang-select page-header__lang-select--rus">Русский</a>

		<div class="page-header__burger-wrapper">
			<span class="page-header__burger"></span>
		</div>

		<span class="page-header__strip"></span>
	</div>
</header>
